...
class Article extends CI_Controller{
    ...
    function simpan_post(){
        ...
        if(!empty($_FILES['userfile']['name'])){
            if ($this->upload->do_upload('userfile')){
                ...
                $this->rental_model->simpan_berita($jdl,$berita,$gambar,$tgl);
    ...
    function lists(){
        $x['hasil']=$this->rental_model->get_all_data('artikel');
    ...
    function edit($id=0){
        if($_SERVER['REQUEST_METHOD']=="POST"){
            $data_model=$this->rental_model->get_by_id('artikel','id_artikel',$id);
                if (count($data_model)>0){
                    ...
                    if (empty($gambar))
                    {
                           ...
                           $aksi = $this->rental_model->update('artikel','id_artikel',$id,$data);
                            if ($aksi) {
            ...
            {
                    $data['hasil'] = $this->rental_model->get_by_id('artikel','id_artikel',$id)->row();
                    $tmp['content'] = $this->load->view('admin/backend/artikel/u_artikel',$data, true);
                    $this->load->view('admin/template/bootstrap',$tmp);
            }
    }
